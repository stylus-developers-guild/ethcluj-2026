...
contract TokenVault {
    ...
    function flashLoan(
        ...
        IFlashBorrower(_borrower).onFlashLoan(_amount, _data);
...
contract FlashBorrower {
    ...
    function onFlashLoan(uint256 _amount, bytes calldata _data) external {
        ...
        // 1. Approve dex to spend ClujUSD, then swap ClujUSD (token1) for WETH (token2)
        token.approve(address(dex), amountIn);
        dex.swapToken1ForToken2(amountIn);

        // 2. Approve manager to spend WETH, then deposit the WETH received from the swap
        uint256 wethReceived = manager.weth().balanceOf(address(this));
        manager.weth().approve(address(manager), wethReceived);
        manager.deposit(wethReceived);

        // 3. Mint new ClujUSD against the deposited WETH collateral
        manager.mint(_amount);
